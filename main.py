#                        Home Task 6
#  Task 1 Предыдущий проект (Заказ продуктов в магазине) дополнить итерационным протоколом.
...
class Cart:
    ...
    def __next__(self):
        products_list = list(self.products.items())
        if self.index < len(products_list):
            current_product, quantity = products_list[self.index]
            self.index += 1
            return f"{current_product.name}: {current_product.description} - {quantity}"
        else:
            raise StopIteration
